...
**Note:** There is NO live 2-player mode in the current menu. Live mode is **DORMANT** — see "Live Mode (Dormant)" section below.
...
| `js/core/SceneNavigator.js` | Scene navigation via emotion-space geometry (DONE, wired to archive) |
| `js/core/ContaminationTracker.js` | Per-axis contamination tracking (DONE, wired to archive) |
...
| `js/index.js` | Main app entry, orchestrator |
| `js/app/archive.js` | Archive/Play mode — engine-based navigation pipeline |
...
| `js/app/live.js` | **DORMANT** — 2-player live mode (not in active menu, do not wire to engine) |
...
### Done
1. **SceneNavigator.js** — center shift + radius, fallback B with narrative ✅
2. **ContaminationTracker.js** — 3-axis vector + EMA drift/fixation + stage mixing ✅
3. **Archive mode integration** — SceneNavigator + ContaminationTracker wired into archive.js ✅
4. **Test infrastructure** — Vitest, 149 tests covering math/engine/contamination/navigator ✅

### Remaining
1. **Admin panel** — radius simulator, isolated scene warning, contamination dashboard
2. **Record pattern-aware questioning** — AI rhythm changes
3. **Initial telling trajectory storage**

### Future (not needed yet)
- **FlowController** — Only needed when a second mode (e.g. Live) shares the same engine pipeline. Currently archive.js owns the full pipeline (`collectEmotionInput → runEngineStep → applyEngineResult → proceedToNextScene`) and is the sole consumer. Do not extract prematurely.
- **Live mode engine integration** — Live mode uses intentional linear `currentScene + 1`. Dormant; will only be connected to the engine in a dedicated phase.

## Live Mode (Dormant)

**Status:** NOT ACTIVE. Not accessible from the current menu. Do not connect to ByeoriEngine/SceneNavigator/FlowController.

- **Files:** `js/app/live.js` (2,925 LOC), live-related functions in `js/index.js`
- **What it is:** A 2-player real-time session mode (narrator + experiencer) with Supabase Realtime subscriptions
- **Why dormant:** Requires dedicated UX design, separate Realtime infrastructure, and a different interaction model from single-player archive traversal
- **Linear progression (`currentScene + 1`) is intentional** in live mode for now — do not replace with SceneNavigator
- **When to activate:** Only when explicitly requested. Will need its own FlowController variant that handles two simultaneous users

## Two Parallel Implementations (Known Tech Debt)

- **Main path** (`index.html` → `js/app/archive.js`): Engine-based navigation via SceneNavigator
