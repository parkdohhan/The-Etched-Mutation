...
### 작업 7 — 메모리 1~2개 큐레이션 [1 세션]
- 기존 메모리 중 Lumen 데모에 적합한 것 선택 or 단일 신규
- `terrain_shape = circular`, `ghost_condensation_points` 2~3개 수동 입력
- Admin UI 없으므로 SQL 직접 (또는 JS 상수 결정 시 `js/shared/ghost_points.js`에 직접 입력)
...
| 구간 | 기간 | 내용 | 세션 |
|---|---|---|---|
| Week 0 (일본) | 4-21~22 | 문서 · DB SQL · 메모리 구상 · 귀환 컴포넌트 정의 | 0.8 |
| 귀국/시차 | 4-23 | 휴식 | — |
| Sprint 1 | 4-24~26 | 작업 0 + 3 + 3b + 3c + 11 | 6.3 |
| 통합일 | 4-27 | 본인 한 바퀴 + 버그 리스트 (코드 금지) | — |
| Sprint 2 | 4-28~30 | 작업 1 + 4 + 7 + 12 | 4.3 |
...
**총 코드 세션**: 14.1 (원안 13.3 + 작업 12 0.8)
**총 기간**: 28일 (4-22 ~ 5-19)
