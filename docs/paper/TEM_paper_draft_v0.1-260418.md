# 기억유전학적 관점에서의 감정 궤적 측정: The Etched Mutation의 시스템과 메트릭

**Measuring Emotional Trajectory Divergence in Interactive Narrative: The Etched Mutation System and Its Metrics**

Dohhan Park

Working Draft v0.1 — 2026-04-18

> **이 드래프트의 포지션**
> 본 드래프트는 2026-04-03 통합 초안([mnemonic_genetics_paper_draft_ko-260403.md](./mnemonic_genetics_paper_draft_ko-260403.md))을 재구성한 것이다. 2026-04-18 스코프 결정에 따라, 분자유전학·레트로바이러스 생활사와의 구조적 유비(Arc 단백질, HIV-1 계통역학, Central Dogma 대응)는 본 논문에서 제외한다. 기억유전학은 본 작업이 발원한 **개념적 계보(intellectual lineage)**로 명시하되, 그 생물학적 정당화에는 의존하지 않는다. 본 논문은 **시스템·메트릭·궤적 비교**의 계산적·체험적 타당성만을 주장한다.

---

## Abstract

상호작용 서사에서 서로 다른 체험자는 서로 다른 경험을 하지만, 그 "다름"은 대부분 플롯 분기의 차이로만 기술되어 왔다. 본 논문은 **감정 궤적(emotional trajectory)을 상호작용 서사의 일차 차원으로 두는** 시스템 *The Etched Mutation (TEM)* 과, 체험자 궤적이 작가 원본 궤적으로부터 이탈하는 양상을 두 가지 분리 가능한 양식 — **drift (방향 이탈)** 과 **fixation (단일점 수렴)** — 으로 측정하는 오염 벡터(contamination vector), 그리고 궤적 형태의 유사도를 실시간으로 산출하는 별이엔진 V4(Byeori Engine V4)의 정렬도(alignment) 메트릭을 제시한다. 본 시스템은 명시적 분기 구조 없이 매 턴 체험자의 감정 상태에 따라 접근 가능한 장면 집합(accessible pin set)을 계산하며, 공명(resonance)에 도달한 체험자의 궤적은 다음 체험자를 위한 해석 조각(*trajectory bridge*)으로 자동 변환된다. 193 + 100 건의 페르소나 시뮬레이션 플레이로 메트릭의 내부 타당성을 시범 검증한다. 본 작업은 저자가 **기억유전학(mnemonic genetics)** 이라 명명한 개념적 관점에서 출발하였으며, 해당 관점의 온전한 이론적 전개는 별도 작업물에서 다룬다. 본 논문의 기여는 관점의 증명이 아니라, 그 관점이 요구했으나 아직 제공되지 못한 **측정 장치와 체험 시스템**이다.
...
**첫째, 시스템적 기여.** *The Etched Mutation (TEM)* — 감정 벡터를 씬(scene)의 일차 속성으로 두고, 매 턴 체험자 감정 상태에 따라 접근 가능 씬 집합을 계산하는 인터랙티브 서사 시스템. 3D 감정 지형(*strata*) 위에 씬이 VAD 좌표로 배치되며, 공명 도달자의 궤적이 **trajectory bridge** 로 자동 축적된다.

**둘째, 방법론적 기여.** 두 가지 계산 가능한 메트릭.
1. **별이엔진 V4 정렬도 (Byeori Engine V4 alignment)** — 체험자 감정 시퀀스와 작가 원본 감정 시퀀스의 형태 유사도를 실시간으로 산출.
2. **오염 벡터 (contamination vector)** — 궤적 이탈을 **drift (방향성 누적 이탈)** 와 **fixation (단일 귀인·감정으로의 과도한 수렴)** 두 축으로 분리 측정. 두 축은 동일 총 이탈량이라도 체험적으로 다른 양상을 구분한다.

**셋째, 시범 검증.** 193 + 100 플레이 규모의 페르소나 시뮬레이션으로 메트릭의 내부 일관성과 페르소나 전략 간 분별력을 검증한다.

### 1.3 이 작업이 놓인 개념적 계보 — 기억유전학

이 작업의 용어(오염, 정렬도, 변이, 궤적 브릿지, 공명)와 설계 결정은 저자가 **기억유전학(mnemonic genetics)** 이라 명명한 관점에서 발원하였다. 그 관점의 핵심 직관은 세 가지다. (1) 기억은 보존되는 것이 아니라 **변형되며 전파된다.** (2) 이 변형은 열화(degradation)가 아니라 **재창조(recreation)** 로 읽힐 수 있는 6가지 유형의 연산으로 분류할 수 있다 — 복제, 변이, 선택, 번역, 수선, 재조합. (3) 이 변형의 양상은 **측정 가능**해야 한다.

그러나 본 논문은 이 관점의 온전한 이론적 전개 — 특히 그것이 분자유전학의 연산 체계와 어떤 관계에 있는지 — 를 다루지 않는다. 해당 작업은 저자의 별도 저술에서 다루어지며, 본 논문은 그 관점이 요구했으나 아직 제공되지 못한 **측정 장치, 체험 시스템, 그리고 체험자 간 비교의 계산적 기반**만을 기여로 제시한다. 읽는 이는 본 논문의 시스템과 메트릭을 그 자체의 계산적·체험적 메리트로 평가할 수 있으며, 개념적 계보의 수용 여부와 무관하다.
...
이 풍부한 축적에도 불구하고, 이 현상들은 대체로 **개별 현상으로 다루어져 왔다**. 재고화, 오정보 효과, 거짓 기억, 언어적 가림, 기분 일치 왜곡이 *왜 동시에 존재하는지*, 이들이 하나의 동역학의 서로 다른 표현인지 독립적 메커니즘인지에 대한 통합적 프레임은 부재하다. 기억유전학 관점은 이 현상들을 6개 연산(파괴적 복제, 편향적 변이, 의도적 선택, 무규칙 번역, 과잉 수선, 재조합)의 **유형론(typology)** 으로 묶는 저자의 제안이다. 본 논문은 이 유형론의 완전한 정당화를 시도하지 않으며, 유형론이 **TEM의 설계 결정에 어떻게 반영되었는지** 만을 §3에서 간략히 짚는다.
...
그러나 CompTIES는 비디오 rating dial 기록에 모델을 **사후에 피팅**하는 관찰 데이터 분석 도구이며, 실시간 인터랙티브 환경에서 체험자의 감정 입력에 즉각 반응하며 원본 궤적과 비교하는 도구는 확립되어 있지 않다. 본 논문의 별이엔진 V4(§5.1)는 TIES의 핵심 원리 — *궤적의 동적 패턴이 수준(level)과 독립적으로 유사성을 예측한다* — 를 상호작용 서사의 **실시간 내부 엔진**으로 이식한 첫 시도이다.
...
| 선행 연구 | 기여 | TEM과의 관계 |
|---|---|---|
| Bartlett(1932), Nader et al.(2000) 등 | 기억 변형 메커니즘 규명 | 개별 현상 → 본 논문은 TEM 내부 유형론으로만 참조 |
...
시스템은 vanilla JavaScript ES6 모듈, Three.js (3D), Supabase (인증·DB·Edge Functions), HRTF 기반 Web Audio API로 구현되어 있다. 회귀 테스트 인프라로 vitest 기반 261 케이스가 상시 실행된다.
...
체험자가 작품의 핵심 정서적 지향점 — 저자가 "공명(resonance)"이라 부르는 조건 — 에 도달하면, 그 체험의 궤적이 자동으로 **trajectory bridge**로 변환된다 ([docs/데이터계약_브릿지_v1-260412.md](../데이터계약_브릿지_v1-260412.md)). 이는 자동 승인되며 다음 체험자의 strata와 씬 렌더링에 노출된다.
...
즉 궤적 브릿지는 **한 체험자의 기억-속-기억**이다. 작가의 정적 해석(author_bridge)과 공존하되, **누적성·익명성·감정 축 기반 노출**이라는 점에서 구분된다. 노출 메커니즘은 인기도가 아니라 현재 체험자의 감정 상태와의 정렬도(§5.1)이다.
...
[체험자 감정 입력]
        ↓
  ByeoriEngine (정렬도 계산, §5.1)
        ↓
  ContaminationTracker (오염 벡터 갱신, §5.2)
        ↓
...
## 4. 기억유전학 관점이 시스템 설계에 반영된 지점

§1.3에서 언급했듯, TEM의 용어와 설계 결정은 저자가 **기억유전학**이라 명명한 개념적 관점에서 발원했다. 본 절은 그 관점의 완전한 이론적 전개 대신, **관점이 시스템의 어떤 결정에 실제로 영향을 주었는지**를 추적 가능한 방식으로 나열한다. 이 매핑은 독자가 관점을 수용할 것을 요구하지 않으며, 단지 시스템의 용어가 어디서 왔는지를 투명하게 하기 위함이다.

| 기억유전학 개념 | TEM 시스템 구현 | 근거 코드/문서 |
|---|---|---|
| 기억은 회상될 때마다 변형된다 | 매 턴 체험자 감정 상태가 궤적에 누적되고, 같은 작품이 매 체험마다 다르게 진행됨 | SceneNavigator |
| 변형은 6개 연산의 유형론으로 분류 가능 | ContaminationTracker가 echo_follow / bridge / contradiction / displacement / avoidance / fixation 전이 패턴으로 분류 | contamination/ |
| 변형의 일부는 측정 가능 | drift / fixation 두 축의 contamination vector | §5.2 |
| 기억은 **의존적 복제자** — 다른 의식 없이는 연산이 수행되지 않음 | 궤적 브릿지가 공명 도달자 없이는 생성되지 않음. 작가 brid만으로는 작품이 닫힌 계로 남음 | §3.5 |
| 이본론: 변이 = 재창조, 오염 ≠ 열화 | 오염(contamination) 지표가 높은 체험도 부정적으로 표시되지 않으며, 일정 조건에서 trajectory_bridge로 상승 | 자동 승인 로직 |

"오염(contamination)"이라는 용어 선택은 의도적이다. 일반적 함의 — *더러움, 열화* — 를 정면으로 역전시키는 시도이며, 이 역전은 **이본론**이라 저자가 명명한 입장의 핵심이다. 본 논문은 이본론을 철학적으로 완전히 전개하지 않으나, 시스템의 사용자 경험 결정(오염이 체험자에게 경고가 아니라 *감각*으로 전달됨)에 이 입장이 반영되어 있음을 투명하게 해 둔다.

---

## 5. Metrics

### 5.1 Byeori Engine V4 — Trajectory Alignment
...
- **단조성 불보장**: 단일 턴에서 정렬도가 하락할 수 있음(이는 설계 결정이며 §7에서 논의).

### 5.2 Contamination Vector — Drift & Fixation
...
현재 코드는 `signal_sim` 한 축만 구현되어 있으며, 전체 복합 신호 구현은 진행 중이다([docs/메모/Feedback.md](../메모/Feedback.md) §우선순위 A-1). 본 논문의 §6 평가는 복합 신호 기준으로 수행한다.

#### 5.2.3 왜 두 축인가 — 분별력의 동기
...
본 논문의 평가는 **페르소나 시뮬레이션 데이터**로 수행된다. 실제 인간 체험자 연구는 범위 밖이며 별도 후속 작업에서 다룬다(§7.3).
...
- **인지심리학적 주장**: §4의 기억유전학 유형론이 본 메트릭으로 "증명"되었다고 주장하지 않는다. 메트릭은 관점이 요구한 측정 장치이며, 그 자체가 관점의 타당성 증명이 아니다.
...
본 논문은 다음을 주장하지 **않는다**: 기억유전학 관점의 이론적 정당화, 분자유전학과의 수학적 대응, 인간 체험자에 대한 외적 타당성, 어떤 궤적이 "올바른" 궤적인지에 대한 규범적 판단.
...
- **별도 트랙**: 기억유전학 관점의 이론적 전개 및 quasispecies-style 수학적 기반은 저자의 별도 저술과 향후 박사 작업에서 다룬다. 이 트랙은 본 논문의 **전제가 아니라 귀결** 이며, 본 논문의 측정 장치가 성립한 이후에만 의미 있는 이론화로 이어진다.
...
이 작업은 저자가 **기억유전학**이라 명명한 개념적 관점에서 발원했으나, 본 논문의 기여는 그 관점의 증명이 아니라, 관점이 요구했던 **측정 장치와 체험 시스템** 자체이다. 관점의 이론적 전개는 별도 저술에서 이어진다. 시스템은 관점에 의존하지 않고 자립하며, 독자는 본 논문의 메트릭과 시스템을 그 자체의 메리트로 받아들이거나 반박할 수 있다.
...
## 다음 작업 (v0.1 → v0.2 체크리스트)

- [ ] **§6 평가 가설 H1/H2/H3 실제로 돌려보기** — persona-sim 데이터로 숫자 뽑기. 결과에 따라 §6 본문 수정.
- [ ] **§4 매핑 표의 "기억유전학 개념 ↔ 시스템 구현" 각 행별 근거 코드 링크 보강**
- [ ] **§5.1 alignment 수식 실제 코드 대조** — `js/shared/math.js` 확인 후 수식 일치 확인
- [ ] **§2.2/2.3 인용 보강** — Short, Evans & Short, Burford 정확한 출처 확인
- [ ] **§6.4 P-07 전문 부록 A** — 실제 한 페르소나 궤적 추출 후 삽입
- [ ] **§3 Figure 1 (아키텍처 다이어그램), Figure 2 (strata 스크린샷), Figure 3 (trajectory bridge 렌더링 예)**
- [ ] **제목 최종 결정** — 현재 "기억유전학적 관점에서의..." — 심사자가 생물 formalism 기대할 위험. §1.3만으로 관점이 충분히 framing 되는지 검토 후 더 중립적 제목 고려 ("Emotional Trajectory Divergence in Interactive Narrative: The Etched Mutation System")
