...
- [x] V2.1.2 (δ-2) oneScene + 백그라운드 — first 진입 timeout fix (5-06). `당신에게` (11 씬) timeout 자리 발견 → `oneScene=true` 옵션 자리 박음 (max_tokens 1500 / timeout 8s) + 백그라운드 자리 통째 호출 (max_tokens 8000 / timeout 45s) 동시. 사용자 체감 자리 첫 씬 ~3-5초, 두 번째 씬부터 캐시 hit 즉시
- [x] V2.1.2 (ε) 학습된 유령 자유 대화 — `dialog-turn` edge function + 시스템 프롬프트 자리 = 씬 본문 통째 + ghost_variants drift 자료 (작가 시드 + 이전 회차 자생 변주, 최대 30개) + `cache_control: ephemeral` (90% 비용 절약). turn loop 자리 갱신: scene_context 5-6→2-3 문장 / turn 1 = 선택지+자유입력 동시 (`_renderChoicesOrInput`) / turn 2-3 = 자유입력만 / 응답 = `_callDialogTurn` LLM / 슬롯 흡수 = 백그라운드 (§2 명제 보존) / pickResponse = LLM 실패 시 안전망. **작가 손 검증** (5-06) — *공원에서* 메모리 (memId=c4888189) 진입 = turn 1/3 응답 "할아버지가 자꾸 그 공원을 보고만..." (씬 본문에서 자연), drift row 자생 자리 6개 박힘 (208f03ed/a0dd331b/dfcd987c/2b2ae7c3/0cc398a6/4eea4410), 두 번째 씬 cache hit 즉시. 작품 명제 §2 ("관객 파편 → 다음 관객 유령") 진짜 작동.
...
**V2.1.2 (δ-2) oneScene + 백그라운드 — first 진입 timeout fix (2026-05-06 완료)** — δ 자리 *당신에게* (11 씬) 메모리 자리 first 진입 자리 LLM timeout (15초 자리 안 자리 응답 X) 발견. 사용자 결정 = *2번 옵션* (첫 씬만 빠르게 + 백그라운드 통째). (1) `generate-dialog-choices` 갱신 — `oneScene: boolean` 옵션 박음. true 시 `scenes.eq('id', sceneId)` 자리 그 씬 1개만 SELECT + max_tokens 1500 / timeout 8초 + DB UPDATE skip (race 회피). false 시 통째 SELECT + max_tokens 8000 / timeout 45초 + DB UPDATE. buildUserPrompt = 동적 N개. (2) `lumen_dialog_phase1.js` start() 두 호출 동시 — (A) `await` `oneScene=true` 자리 (~3-5초) → 첫 씬 자리 즉시 진입 자리. (B) `fire-and-forget` 통째 자리 → 백그라운드 자리 ~15-30초 자리에 DB 캐시 자리 박힘 자리. 다음 씬 자리 진입 = 캐시 hit. 운영 자리 deploy 끝. 코드 ~30분 (5-06 완료).

**V2.1.2 (ε) 학습된 유령 자유 대화 — 임기응변 → 학습 (2026-05-06 완료)** — 사용자 결정 = 작가 박은 응답 풀 자리 + 슬롯 자리 채우는 자리 임기응변 자리 폐기. 매 턴 자리 LLM (Haiku 4.5) 자리 호출 = 유령 자리 그 씬 자리 본문 자리 통째 자리 학습 자리. 사용자 자리 자유 자리 질문 자리 = 학습된 자료 자리 안 자리 자연 자리 답. (1) **새 edge function `dialog-turn`** (`supabase/functions/dialog-turn/index.ts`) — 시스템 프롬프트 자리 = 메모리 제목 + 모티프 + 그 씬 본문 통째 + **`ghost_variants` drift 자료 자리 (작가 시드 + 이전 회차 자리 자생 변주, 최대 30개)**. `cache_control: ephemeral` 자리 박음 → 같은 씬 멀티턴 자리 90% 비용 절약 + 두 번째 턴 자리부터 ~1-2초 자리. messages = dialogHistory 자리 누적 + 새 입력 자리. temperature=0 / max_tokens=200 / timeout 10초. (2) `lumen_dialog_phase1.js` turn loop 자리 갱신 — scene_context 5-6 → 2-3 문장 자리. turn 1 = 새 helper `_renderChoicesOrInput` 자리 (선택지 + 자유 입력 동시 박는 UI). turn 2-3 = 자유 입력만. 응답 자리 = `_callDialogTurn` (LLM). 슬롯 흡수 자리 = 백그라운드 자리 (`(async function()`) — §2 명제 자리 보존 자리. dialogHistory 자리 누적. 풀 픽 (`pickResponse`) = LLM 실패 시 안전망만. choice_reply / free_dialog_open 폐기 (작가 의도 자리 = ghost_variants 시드 자리 시스템 프롬프트 자리에 박힘 자리로 보존). (3) **§2 명제 자리 진짜 작동**: 사용자 자리 입력 자리 → SlotAbsorber 자생 자리 → ghost_variants drift row 자리 → **다음 플레이어 자리 dialog-turn 시스템 프롬프트 자리에 자료 자리로 박힘** → 다음 유령 자리 *이전 사람 자리 박은 자리* 알고 답. "관객 자리 파편 자리 다음 관객 자리 유령 자리 흘러간다" 자리. (4) 운영 자리 deploy 끝. 검증 자리 *공원에서* 메모리 (memId=c4888189) — turn 1/3 응답 자리 "할아버지가 자꾸 그 공원을 보고만..." (씬 본문 자리에서 자연 자리), 슬롯 흡수 자리 자생 자리 ghost_variants drift row 자리 6개 자리 박힘 (208f03ed / a0dd331b / dfcd987c / 2b2ae7c3 / 0cc398a6 / 4eea4410), 두 번째 씬 자리 cache hit 자리 즉시 자리. 비용 자리 ~$0.05/회차 (캐시 자리 90% 절약 적용 시 ~$0.015), 지연 자리 첫 턴 ~3-5초, 두 번째 턴부터 ~1-2초. 코드 ~3시간 (5-06 완료).
