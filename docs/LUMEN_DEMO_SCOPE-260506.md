...
> **2026-05-11~13 진행** — V2-6 drift 픽 본 자리. 결정 11개 확정 (temperature 0.8 / N=8 / baseline=회차시작 사용자fp / 직전패널티 ×0.3 / mergeTurn이 누적 담당 / Edge function `record-drift-cumulative` 신설 / drift toast OFF / 비교화면 유령발화는 영상단계 / wiring 둘 다(lumen+archive) / vitest 25 / 폴백 13+13) — 상세 = [세션핸드아웃_v26_drift픽_브레인스토밍-260512.md](세션핸드아웃_v26_drift픽_브레인스토밍-260512.md) §6-X. **파일별 병렬 세션 핸드아웃 = [V2-6_병렬세션_핸드아웃-260513.md](V2-6_병렬세션_핸드아웃-260513.md)** (H1 DriftPicker.js / H2 vitest 25 / H3 Edge function / H4 play-test.html wiring / H5 opening.js wiring / H6 폴백 13+13 / H7 재진입 smoke / H8 V2-5 잔불 선택). **지형 quilt** = V2-5/V2-6/V2-9/V2-12 다 박은 *뒤* 시간 남으면 박는 자리 — 폐기 아님, 후순위.

- [x] V2-5 오염 카메라 연출 — drift 시 즉시 가시화 (1일, 5-6 핵심 / 5-13 잔불 H8 마무리)
  - [x] V2-5 잔불 (핸드아웃 §H8, 2026-05-13 완료) — `js/ui/lumen_visual_effects.js` 단독. (1) drift 카메라 미세 진동: 오염 `cont_depth` 비례 상시 sub-degree 떨림(`camera.rotation` x/z/y 만 가산 — `walk_effects` rotation.y 가산·`_fpTick` rotation.set 리셋과 충돌 없음, sin 2개 합성으로 유기적) + `applyDriftCameraShake(intensity, durationMs)` 펄스 호출 자리 노출 + `__lumenDriftVisualizer` 펄스 rising-edge 자체 폴 → 결 어긋나는 턴에 burst (`dialog_phase1.js` 무수정). (2) drift 조명 색조: `cont_depth` 비례 매우 옅은 cool tint DOM overlay(#lumenDriftTint, max 0.06, vignette 바로 아래 z=238). 둘 다 `driftShakeEnabled`/`driftTintEnabled` 로 끔. 강도 시드 = V2-12 튜닝 대상. `play-test.html` 은 이미 `contaminationDepthProvider` 와 함께 attach 하므로 추가 wiring 없음. 검증 playwright (5-13): 모듈 로드 / `applyDriftCameraShake` 노출 / fog·tint·camera.rotation 모듈레이션 / FP 밖 fog 원복 / detach 시 overlay 제거 / 콘솔 에러 0.
...
- [x] **V2-6 drift 픽 시스템** (2026-05-13 박힘, 작가 손 1사이클 검증 미완) — 12축 emotion_vec 2단계 픽 + `plays.ghost_variant_id`·`final_drift_vector` 도장. 파일별 핸드아웃 = [V2-6_병렬세션_핸드아웃-260513.md](V2-6_병렬세션_핸드아웃-260513.md). 결정 11개 = [세션핸드아웃_v26_drift픽_브레인스토밍-260512.md](세션핸드아웃_v26_drift픽_브레인스토밍-260512.md) §6-X.
  - [x] H1 `js/core/DriftPicker.js` — `pickDriftUtterance` 5단계 (글로벌 좁힘 N=8 → 의미 필터 점진 풀기 → softmax T=0.8 sampling → 직전 패널티 ×0.3 → 폴백) + `DRIFT_PICKER_DEFAULTS`. H2 `test/smoke_v21_drift_picker.test.js` vitest 25 PASS.
  - [x] H3 `supabase/functions/record-drift-cumulative/index.ts` — `memories.cumulative_emotion_vec` EMA 갱신 (service_role 우회, `record-absorption` 패턴). **Supabase 배포 완료** (version 2, ACTIVE, verify_jwt=false).
  - [x] H6 `js/content/narrative_fallback_strings.js` — 폴백 한국어 13 + 영어 13 (`pickFallbackString` / `fallbackPoolSizes`).
  - [x] H4 wiring — `play-test.html` (import `computeDriftVector` + `DriftPicker` self-attach + `NarrativeFallback` 노출 / `game._runStartTime`·`driftStamp` / `insertPlay` `ghost_variant_id` 컬럼 / `sealBtn` 회차 끝 도장 = plays UPDATE `ghost_variant_id`+`final_drift_vector` + `record-drift-cumulative` invoke + `sessionStorage.tem_final_drift_vector`/`tem_last_variant_id`) + `js/ui/lumen_dialog_phase1.js` (`_loadAndInjectGhostPools` — SELECT `motif_tags`/`attribution` 추가 + 회차당 1회 drift 픽 캐시 `_runDriftPick` + `cumulative_emotion_vec` SELECT + `pickDriftUtterance` 호출 → `resonancePool` 맨 앞 주입 = 즉시 가시 / 폴백 시 `narrative_silence` → 폴백 문구 한 줄).
  - [x] H5 — archive 흐름은 `opening.js`에 `decideBranch` wiring 이미 박힘 (V2-4). drift 픽은 메모리 진입 후(play-test.html = H4) 일어나고 archive 흐름도 결국 play-test.html 진입 → H4 가 커버. 추가 wiring 없음.
  - [x] dialog_pipeline.test.js 2개 사전 실패 고침 (테스트가 `pickNextQuestion` 의 `_askedSlots` mutate 를 무시하고 같은 fp 재사용 → 각 호출마다 새 fp 로). 전체 vitest 451 PASS.
  - [ ] **작가 손 1사이클** (dev 서버 — 첫 회차 → 닉네임 → 같은 메모리 두 번째 회차 → 콘솔에서 `plays` 두 행 `ghost_variant_id` 서로 다른 ID + `final_drift_vector` 12축 부분 객체 + `memories.cumulative_emotion_vec` 갱신 확인. screens-260513-V2-6/ 박음). V2-12 튜닝 항목: temperature 0.8→0.5~0.7 / narrowN 8 / lastPenalty 0.3 / transition_pattern 픽 입력 여부.
  - [ ] H8 `js/ui/lumen_visual_effects.js` V2-5 잔불 (선택 — `applyDriftCameraShake` + drift tint, 코드 리뷰 ok, 본인 판단으로 보강 두 자리로 충분하면 SCOPE V2-5 `[x]`).
  - [ ] H7 `test/smoke_v21_re_entry.js` — V2-13 재진입 회귀 가드 (DevTools 콘솔형, 코드 리뷰 ok, 실행 검증은 dev 서버에서).
- [x] **V2-13 재진입 유도 시퀀스** (5-05 V2.1.3 신규, 5-09 확장, 5-10 박힘) — outro 뒤 장면화 + Continue + 닉네임 인라인 입력(localStorage `tem_user_name`) + 첫/두 번째 파동 비교 (`comparison.js` 핵심 함수 재용도화 → `js/app/playReplayCompare.js` 신설) + 메뉴 정리(PLAY 라벨 → ARCHIVE, 최상단) + ARCHIVE 메모리 클릭 시 툴팁(감각 설명 + 지형 PNG 미리보기) + 재진입 시 "뭔가 달라졌어" 멘트 + 흡수 cinematic 가시화 + localStorage 추적 (1.5~2일, 5-9 ~ 5-10)
  ...
  - [x] **play-test.html (lumen) outro 자리 통합** (2026-05-10 완료) — `_v21BranchK` 죽은 게이트 제거 + `game.traces` 기반 wiring (commit b7142bd) + `game.completedSentence = data.narrative` 박음 (commit aaf11f4, `play-test.html:4423` 박힘 — narrative ok / fallback / catch 3 자리). 검증: playwright 두 사이클 (5-10) — handoff probe `tracesLength: 3, filteredScenesLength: 3` / `firstPlayScenification` ("너의 이름을 알려줘" + saveFirstPlayData → localStorage row) / `playReplayCompare` ("re-engraving"). **5-10 추가 검증** = critic 자리에서 placeholder 시각 자리 짚어 한 사이클 더 박음 — `07_playReplayCompare_FIXED.png` + `08_firstPlayScenification_FIXED.png` 자리에 *진짜 봉인 narrative* 박힘 (placeholder 영문 갈아끼움 ✓). 스크린샷 8장 = `screens-260510-V2-13/`.
  - [x] ARCHIVE 메모리 카드 툴팁 (2026-05-10 완료) — `js/ui/UIManager.js` 카드 렌더 자리 갈아끼움 (inline `onclick` 제거 + 두 단계 click handler) + `css/index.css` `.memory-card.expanded` + `.memory-card-tooltip` 박음. UX: 첫 클릭 = expand (감각 한 줄 + 지형 placeholder + 봉인 문장은 *플레이 박은 메모리만*) / 두 번째 클릭 = `selectMemory` / 다른 카드 클릭 = 자기 collapse / 외부 클릭 = 모두 collapse. 감각 fallback 우선순위: `meta.sensory_description` → `memory_words.slice(0,5).join(' · ')` → "감각이 박히지 않은 자리". lang resolve = `localStorage.tem_language`.
  - [x] PLAY 메뉴 진입 자리 = 오프닝 흐름 그대로 재호출 (2026-05-11 완료, 5-13 wave 보강) — 옛 자리 `archive.js _initMemoryFinder` (두 줄 멘트 + finderResultPhase 6개 도어 늘어선 자리) 라우팅 끊음. 새 자리 = `opening.js enterPlayReplaySequence` (신설 export) → openingScreen 다시 띄움 + langGate/닉네임 자리 건너뜀 + `_runV2Sequence` 호출. 흐름: replayIntro 한 줄 `...다른 걸 찾고있어?` + 멀티턴 인터뷰 (턴 1~3) + ASCII 도어 한 짝 빨림 + play-test.html 점프. 근거 = §6.5 #5 "Player sees fog" (선택권 = 전략 게임 격하 위험). 띄어쓰기 교정 = `'...다른걸'` → `'...다른 걸'` (`opening.js V2_DIALOGUES.ko.replayIntro` + `archive.js PLAY_ENTRY_DIALOGUES.ko[0]`). 옛 finder 자리 (`_initMemoryFinder` / `_finderMatch` / `_showFinderResults`) 코드는 살림 — 롤백 자리. **5-13 wave 보강**: 오프닝 기본 wave (`openingWaveCanvas`, transform scale(5,1) 푸른 톤) 는 viewport 에서 캔버스 가운데 1/10 만 보여 거의 안 보임 → PLAY 재진입 자리에선 숨김(`display:none`) + RAF 정지하고, `opening.js _mountOpeningWarmThread` (옛 finder `_mountFinderThread` 와 동일한 warm amber 7겹 합성파, openingScreen absolute, 멘트 위 top 38%, 높이 150px) 를 박음. waves 배열·tick 로직 두 자리 동기화 필요. 검증 playwright (5-13): warm canvas opacity 1 / maxBright 255 / blue container display none / 칩→멀티턴 흐름 정상.
- [x] **V2-9 통합·smoke 가드** (2026-05-13 박힘) — 풀 사이클 통합 vitest. 단위 가드 6개(`smoke_v21_match_engine` / `_dialog_pipeline` / `_branch_trigger` / `_phase1` / `_drift_picker` / `_lumen_outro`) + V2-13 재진입(`smoke_v21_re_entry.js` DevTools 콘솔형) + 본 풀 사이클 (`test/smoke_v21_full_cycle.test.js`, 10 케이스). 시나리오 = ①오프닝 3턴 fp 누적 → ②`pickTopMemory` → ③`pickGhostVariant` → ④`decideBranch` drift → ⑤`pickDriftUtterance` 1회차 변주 → ⑥`computeDriftVector` + plays 도장 row → ⑦`updateCumulativeEmotionVec` EMA → ⑧재진입 `lastPenalty` → ⑨폴백 `narrative_silence` + 폴백 문구 → ⑩메모리간 격리 가드. 영상 컷 직전(5-17~) 회귀 차단. **전체 vitest 461 PASS.**
...
- [ ] V2-11 데모 trajectory 시각화 페이지 (5-14 신설) — admin 안 끌어옴, `demo-trajectory.html` + `js/demo/trajectory-demo.js` 신규. 별이엔진 + sceneNavigator 직접 호출. A/B runner 비교 자동 ON · 자동 재생 · strata 좌표계 · 큰 패턴 라벨 오버레이. **영상 컷·발표 시연용 매커니즘 자랑 화면**. 본체 admin 영향 없음. 1~2시간 규모.
...
- [ ] drift 가시 변형 (어휘·발화 + 카메라 연출)
  - [x] 카메라 연출 = V2-5 보강 파동 부활 (5-06 commit 11038ac) + V2-5 잔불 H8 카메라 미세 진동·조명 색조 (5-13, `lumen_visual_effects.js`)
...
- [ ] **재진입 시 발화 변화 시연** (V2.1.3 트리오 — V2-6 + β + V2-13) — V2-13 영상 컷 자리에서 본인 손 가능
  - [x] V2-13 시스템 자리 작동 검증 (2026-05-10 playwright 두 사이클) — first/second play 분기 박힘, scenes·userEmotion·userName·completed_sentence 자료 박힘. 스크린샷 `screens-260510-V2-13/`.
  - [ ] 영상 컷 본인 손 (5-17 ~ 5-19 자리)
- [ ] 영상 + 스크린샷 + statement
- [ ] Lumen 제출 양식
