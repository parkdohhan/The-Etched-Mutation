 ...
 *   window.restart, window.stopAllAnimations, window.showNpcDialogue,
...
function collectChatMessage(inputId) {
    ...
    if (!input || !input.value.trim()) {
        window.showNotification('Please enter a message');
        return null;
    }
...
async function sendExpChatMessageWithDeps(deps = {}) {
    const {
        ...
        setExpPendingEmotion = (msg) => { window.appStore.setState({ expPendingEmotion: msg }); }
    } = deps;
...
async function sendChatMessageWithDeps(deps = {}) {
    ...
    if (getCurrentPhase() === 'scene') {
        ...
        try {
            ...
            addMsgFn('ai', errorMessage);
            window.showNotification('Error in AI response');
        }
    }
}
...
async function handleConfirm(answer) {
    ...
    if (currentPhase === 'emotion') {
        if (answer === 'yes') {
            ...
            window.showNotification('Scene sent.');
...
async function saveExperiencerChoice(emotionVector) { console.log('=== saveExperiencerChoice called ==='); console.log('emotionVector:', JSON.stringify(emotionVector)); const appStore = window.appStore; const state = appStore.getState(); console.log('currentSessionId:', state.currentSessionId); console.log('liveSceneNum:', state.liveSceneNum); if (!state.currentSessionId) { console.error('currentSessionId가 not found!'); return } let userId; if (state.currentUser) { userId = state.currentUser.id } else { if (!window.anonymousUserId) { window.anonymousUserId = crypto.randomUUID() } userId = window.anonymousUserId } const insertData = { live_session_id: state.currentSessionId, scene_id: null, user_id: userId, emotion_vector: emotionVector || { fear: 0, sadness: 0, anger: 0, joy: 0, longing: 0, guilt: 0 }, created_at: new Date().toISOString() }; console.log('choices INSERT 데이터:', JSON.stringify(insertData)); try { const result = await networkService.saveChoice(insertData); if (!result.ok) { console.error('choices INSERT error:', result.error); throw result.error } console.log('체험자 감정 저장 complete:', result.data); window.showNotification('감정이 choices 테이블에 저장 complete') } catch (e) { console.error('saveExperiencerChoice error:', e); window.showNotification('choices 테이블 Save failed: ' + e.message) } }
...
function simulateNarratorInput(sceneText) {
    try {
        ...
        window.showNotification('Error loading scene');
    }
}
