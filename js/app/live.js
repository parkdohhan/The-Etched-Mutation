...
function selectRole(role) {
  try {
    ...
    if (sessionSetupEl) {
      ...
      sessionSetupEl.style.cssText =
        'display:flex !important;z-index:1900 !important;position:fixed !important;top:0 !important;left:0 !important;width:100% !important;height:100% !important'
    }
    if (role === 'A') {
...
function subscribeToScenes() {
  ...
  realtimeService.subscribeToScenes(sessionId, {
    onSceneInsert: (sceneData) => {
      ...
      uiManager.displaySceneForExperiencer(
        sceneData,
        {
        ...
        emotionCueMsg,
        NPC_DIALOGUES.live.sceneArrived,
      )
    },
  })
}
...
async function joinLiveSession() {
  ...
  try {
    ...
    const session = sessions.find((s) => s.session_code === code && !s.experiencer_id && !s.ended_at)
...
async function saveLiveScene(sceneData) {
  ...
  try {
    ...
    await saveSceneToLiveSession({
      text: sceneText,
      emotion_vector: sceneData.emotionAnalysis?.base || {
        fear: 0,
        sadness: 0,
        anger: 0,
        joy: 0,
...
async function startLiveSession() {
  try {
    const state = appStore.getState()
    if (!state.currentSessionId && state.currentRole === 'B') {
      console.warn('Experiencer session ID not found')
      return
    }
    let sessionId = state.currentSessionId
    if (!sessionId && state.currentRole === 'A') {
      sessionId = await createLiveSession()
      if (!sessionId) {
        console.warn('Session creation failed, continuing')
      } else {
    ...
    window.updateUserStats('liveSession', 1)
    const sessionSetupEl = document.getElementById('sessionSetup')
    if (sessionSetupEl) {
      sessionSetupEl.classList.remove('active')
      sessionSetupEl.style.display = 'none'
    ...
    const roleState = appStore.getState()
    if (liveContentEl) {
      if (roleState.currentRole === 'A') {
        liveContentEl.classList.add('narrator-mode')
      } else {
        liveContentEl.classList.remove('narrator-mode')
      }
    }
    const narratorLastChoiceSection = document.getElementById('narratorLastChoiceSection')
    if (narratorLastChoiceSection) narratorLastChoiceSection.style.display = 'none'
    const liveProgressSection = document.getElementById('liveProgressSection')
    if (liveProgressSection) liveProgressSection.style.display = roleState.currentRole === 'A' ? 'block' : 'none'
    const traceLabel = document.getElementById('traceLabel')
    if (traceLabel) traceLabel.textContent = roleState.currentRole === 'A' ? '해석의 흔적' : '기억의 흔적'
    if (roleState.currentRole === 'A') {
      const narratorPanelEl = document.getElementById('narratorPanel')
      if (narratorPanelEl) narratorPanelEl.classList.add('active')
      ...
      if (interpretationTrace && traceContent) {
        ...
        traceContent.textContent = '체험자가 장면을 기다리고있습니다...'
      }
      showNpcDialogue('당신의 기억을 불러오세요. 지금 입력하는 장면이 이 기억의 원본 음각이 됩니다.', 4000)
    } else {
      const experiencerPanelEl = document.getElementById('experiencerPanel')
      if (experiencerPanelEl) {
        experiencerPanelEl.classList.add('active')
        expCurrentPhase = 'waiting'
        appStore.setState({ expPendingEmotion: '' })
        expGeneratedEmotion = ''
        expFinalObject = null
        const expSceneText = document.getElementById('expSceneText')
        if (expSceneText)
          expSceneText.innerHTML =
            '화자가 기억을 불러오고 있습니다<span class="loading-dots"><span>.</span><span>.</span><span>.</span></span>'
        const expEmotionText = document.getElementById('expEmotionText')
        if (expEmotionText) expEmotionText.textContent = ''
        const sceneDisplay = document.getElementById('expGeneratedSceneContent')
        if (sceneDisplay) sceneDisplay.style.display = 'block'
        const emotionDisplay = document.getElementById('expGeneratedEmotionContent')
        if (emotionDisplay) emotionDisplay.style.display = 'none'
        showNpcDialogue('곧 누군가의 Original Memory이 열릴 거야. 그 안에서 네 감정을 솔직하게 남겨줘.', 4000)
      } else {
        showNotification('체험자 패널을 not found')
      }
    }
    window.startAlignmentWaveAnimation()
    setTimeout(() => {
    ...
    showNotification('세션을 시작하는 중 Error occurred: ' + e.message)
  }
}
...
async function sendNarratorInput() {
  console.log('sendNarratorInput called')
  const input = document.getElementById('narratorInput')
  if (!input || !input.value.trim()) {
    showNotification('Please enter a memory')
    return
  }
  const inputText = input.value.trim()
  const sendBtn = document.querySelector('.narrator-send-btn')
  if (sendBtn) sendBtn.disabled = true
  if (sendBtn) sendBtn.textContent = 'AI is converting scene...'
  showNotification('AI is converting scene...')
  try {
    const convertedScene = await generateSceneAI(inputText)
    const liveSceneContent = document.getElementById('liveSceneContent')
    if (liveSceneContent) {
      liveSceneContent.textContent = convertedScene
    }
    const experiencerPanel = document.getElementById('experiencerPanel')
    if (experiencerPanel) {
      experiencerPanel.classList.add('active')
    }
    const traceContent = document.getElementById('traceContent')
    if (traceContent) {
      traceContent.textContent = 'Scene sent to experiencer'
    }
    showNotification('Scene sent to experiencer')
    input.value = ''
    const reasonInput = document.getElementById('narratorReason')
    if (reasonInput) reasonInput.value = ''
    updateLiveAlignment(0.15)
    const liveState = appStore.getState()
    appStore.setState({ liveSceneNum: (liveState.liveSceneNum || 0) + 1 })
    const liveSceneNumEl = document.getElementById('liveSceneNum')
    if (liveSceneNumEl) liveSceneNumEl.textContent = appStore.getState().liveSceneNum
  } catch (error) {
    console.error('sendNarratorInput error:', error)
    showNotification('Scene 변환 중 Error occurred: ' + error.message)
    const liveSceneContent = document.getElementById('liveSceneContent')
    if (liveSceneContent) {
      liveSceneContent.textContent = inputText
    }
  } finally {
    if (sendBtn) {
...
async function handleUnifiedSubmit(providedText) {
  console.log('handleUnifiedSubmit called')
  console.log('=== handleUnifiedSubmit ===')
  console.log('currentPhase:', currentPhase)
  let inputText = ''
  if (providedText) {
    inputText = providedText.trim()
  } else {
    const input = document.getElementById('unifiedInput')
    if (!input || !input.value.trim()) {
      showNotification('Please enter your input')
      return
    }
  ...
  if (currentPhase === 'scene') {
    appStore.setState({ pendingSceneText: inputText })
    addChatMessage('user', inputText)
    try {
      const aiScene = await generateSceneAI(inputText)
      currentGeneratedScene = aiScene
      console.log('currentGeneratedScene (after AI):', currentGeneratedScene)
      const sceneContent = document.querySelector('#generatedSceneContent .generated-text')
      if (sceneContent) sceneContent.textContent = aiScene
      switchGeneratedTab('scene')
      addChatMessageWithConfirm('ai', 'Does this memory feel right?')
    } catch (error) {
      console.error('generateSceneAI error:', error)
      showNotification('An error occurred during scene generation')
      currentGeneratedScene = inputText
      const sceneContent = document.querySelector('#generatedSceneContent .generated-text')
      if (sceneContent) sceneContent.textContent = inputText
      switchGeneratedTab('scene')
      addChatMessageWithConfirm('ai', 'Does this memory feel right?')
    }
    return
  ...
  if (currentPhase === 'emotion') {
    console.log('=== EMOTION PHASE ===')
    console.log('inputText:', inputText)
    addChatMessage('user', inputText)
    let emotionResult = null
    try {
      showNotification('AI is analyzing and converting emotions...')
      emotionResult = await analyzeEmotionWithVector(inputText, '')
      console.log('emotionResult (raw):', JSON.stringify(emotionResult))
    } catch (e) {
      console.error('Emotion analysis failed:', e)
      showNotification('감정 분석 Failed: ' + e.message)
    }
    if (!emotionResult || !emotionResult.generatedEmotion || emotionResult.generatedEmotion === inputText) {
      console.warn('AI emotion conversion failed, using original text')
      emotionResult = {
        generatedEmotion: inputText,
        analysis: emotionResult?.analysis || {
          base: { fear: 0, sadness: 0, anger: 0, joy: 0, longing: 0, guilt: 0 },
          detailed: [],
          intensity: 0.5,
          confidence: 0.3,
        },
    ...
    console.log('최종 emotionResult:', emotionResult)
    const emotionContent = document.querySelector('#generatedEmotionContent .generated-text')
    if (emotionContent) {
      emotionContent.textContent = emotionResult.generatedEmotion
      console.log('생성된 감정 표시:', emotionResult.generatedEmotion)
    }
    switchGeneratedTab('emotion')
    const parsed = parseEmotionInput(inputText)
    const currentState = appStore.getState()
    const sceneText = currentGeneratedScene || currentState.pendingSceneText || ''
    console.log('sceneText for finalSceneObject:', sceneText)
    console.log('currentGeneratedScene:', currentGeneratedScene)
    console.log('pendingSceneText:', currentState.pendingSceneText)
    const voidInfo = {
      sceneVoid: !sceneText || sceneText.includes('기억 안 나'),
      emotionVoid: !parsed.emotion,
      reasonVoid: !parsed.reason,
    }
    finalSceneObject = {
      text: sceneText,
      emotionRaw: parsed.emotion || inputText,
      reasonRaw: parsed.reason || '',
      generatedEmotion: emotionResult.generatedEmotion,
      emotionAnalysis: emotionResult.analysis,
      voidInfo: voidInfo,
    }
    console.log('finalSceneObject 생성:', JSON.stringify(finalSceneObject))
    addChatMessageWithConfirm('ai', 'Does this emotion feel right?')
    return
  }
}

async function generateSceneAI(inputText) {
  try {
    ...
    console.log('generateSceneAI response:', data)
    if (data.scene) {
      window.lastSceneData = { scene: data.scene, voidHint: data.voidHint || '', emotionCue: data.emotionCue || '' }
      return data.scene
    } else {
      throw new Error(data.error || 'Scene 변환 실패')
    }
  } catch (error) {
    console.error('generateSceneAI error:', error)
    throw error
...
async function analyzeEmotionWithVector(emotionText, reasonText, anchorEmotions = null) {
  console.log('analyzeEmotionWithVector called:', { emotionText, reasonText, anchorEmotions })
  try {
    const requestBody = {
    ...
    if (!data.generatedEmotion) {
      console.warn('generatedEmotion이 응답에 없음')
    }
    return data
  } catch (error) {
    console.error('analyzeEmotionWithVector error:', error)
    return {
...
function addChatMessage(role, content) {
  ...
  const label = role === 'user' ? 'me' : 'Another Me'
  messageDiv.innerHTML = `<div class="chat-message-label">${label}</div><div class="chat-message-content">${content.replace(/\n/g, '<br>')}</div>`
  messagesContainer.appendChild(messageDiv)
  messagesContainer.scrollTop = messagesContainer.scrollHeight
}

function addChatMessageWithConfirm(role, content) {
  const messagesContainer = document.getElementById('chatMessages')
  if (!messagesContainer) return
  const messageDiv = document.createElement('div')
  messageDiv.className = `chat-message ${role}`
  const label = role === 'user' ? 'me' : 'Another Me'
  messageDiv.innerHTML = `<div class="chat-message-label">${label}</div><div class="chat-message-content">${content.replace(/\n/g, '<br>')}</div><div class="confirm-buttons"><button class="confirm-btn yes" onclick="handleConfirm('yes')">Yes</button><button class="confirm-btn no" onclick="handleConfirm('no')">No</button></div>`
  messagesContainer.appendChild(messageDiv)
  messagesContainer.scrollTop = messagesContainer.scrollHeight
}

function removeConfirmButtons() {
  const buttons = document.querySelectorAll('.confirm-buttons')
  buttons.forEach((btn) => btn.remove())
}

function addExpChatMessage(role, content) {
  const messagesContainer = document.getElementById('expChatMessages')
  if (!messagesContainer) return
  const messageDiv = document.createElement('div')
  messageDiv.className = 'chat-message ' + role
  const label = role === 'user' ? 'me' : 'Another Me'
...
function addExpChatMessageWithConfirm(role, content) {
  const messagesContainer = document.getElementById('expChatMessages')
  if (!messagesContainer) return
  const messageDiv = document.createElement('div')
  messageDiv.className = 'chat-message ' + role
  ...
  messageDiv.innerHTML =
    ...
    '</div><div class="chat-message-content">' +
    content.replace(/\n/g, '<br>') +
    '</div><div class="confirm-buttons"><button class="confirm-btn yes" onclick="handleExpConfirm(\'yes\')">Yes</button><button class="confirm-btn no" onclick="handleExpConfirm(\'no\')">No</button></div>'
  messagesContainer.appendChild(messageDiv)
  messagesContainer.scrollTop = messagesContainer.scrollHeight
}

function removeExpConfirmButtons() {
  const panel = document.getElementById('experiencerPanel')
  if (!panel) return
  const buttons = panel.querySelectorAll('.confirm-buttons')
  buttons.forEach((btn) => btn.remove())
}
...
function switchGeneratedTab(tab) {
  document.querySelectorAll('.generated-tab').forEach((t) => t.classList.remove('active'))
  document.querySelectorAll('.generated-tab-content').forEach((c) => (c.style.display = 'none'))
  if (tab === 'scene') {
    document.querySelectorAll('.generated-tab')[0].classList.add('active')
    document.getElementById('generatedSceneContent').style.display = 'block'
  } else if (tab === 'emotion') {
    document.querySelectorAll('.generated-tab')[1].classList.add('active')
    document.getElementById('generatedEmotionContent').style.display = 'block'
  }
}

function switchExpGeneratedTab(tab) {
  const sceneDisplay = document.getElementById('expGeneratedSceneContent')
  const emotionDisplay = document.getElementById('expGeneratedEmotionContent')
  if (tab === 'scene') {
    if (sceneDisplay) sceneDisplay.style.display = 'block'
    ...
    if (sceneDisplay) sceneDisplay.style.display = 'none'
...
function extractGeneratedText(aiResponse) {
  if (aiResponse.includes('[SCENE_READY]')) {
    try {
      const jsonStr = aiResponse.substring(aiResponse.indexOf('[SCENE_READY]') + '[SCENE_READY]'.length).trim()
      const data = JSON.parse(jsonStr)
      if (currentPhase === 'scene' && data.scene) return data.scene.text || data.scene
      if (currentPhase === 'emotion' && data.emotion) return data.emotion.text || data.emotion
    } catch (e) {}
  }
  return aiResponse.replace(/\[SCENE_READY\].*$/, '').trim() || aiResponse
}

function parseEmotionInput(text) {
  const parts = text
    .split(/[,.]/)
    .map((s) => s.trim())
    .filter(Boolean)
  return { emotion: parts[0] || null, reason: parts[1] || null }
}

function formatEmotionVector(emotionVector) {
  if (!emotionVector) return 'No emotion'
  const emotions = []
  if (emotionVector.fear > 0.1) emotions.push(`두려움 ${Math.round(emotionVector.fear * 100)}%`)
  if (emotionVector.sadness > 0.1) emotions.push(`슬픔 ${Math.round(emotionVector.sadness * 100)}%`)
  if (emotionVector.anger > 0.1) emotions.push(`분노 ${Math.round(emotionVector.anger * 100)}%`)
  if (emotionVector.joy > 0.1) emotions.push(`기쁨 ${Math.round(emotionVector.joy * 100)}%`)
  if (emotionVector.longing > 0.1) emotions.push(`그리움 ${Math.round(emotionVector.longing * 100)}%`)
  if (emotionVector.guilt > 0.1) emotions.push(`죄책감 ${Math.round(emotionVector.guilt * 100)}%`)
  return emotions.length > 0 ? emotions.join(', ') : 'No emotion'
}
...
async function sendExpChatMessageWithDeps(deps = {}) {
  const {
    ...
    setExpPendingEmotion = (msg) => {
      appStore.setState({ expPendingEmotion: msg })
    },
  } = deps
  ...
  if (getExpCurrentPhase() === 'interpret') {
    ...
    try {
      ...
      const errorMessage =
        e.message && e.message.includes('Response format error')
...
async function sendChatMessageWithDeps(deps = {}) {
  ...
  if (getCurrentPhase() === 'scene') {
    ...
    try {
      ...
      const errorMessage =
        error.message && error.message.includes('Response format error')
          ? 'Response format error, please try again.'
...
async function handleConfirm(answer) {
  ...
  if (currentPhase === 'emotion') {
    if (answer === 'yes') {
      ...
      console.log('currentGeneratedScene:', currentGeneratedScene)
      const currentStateForLog = appStore.getState()
      console.log('pendingSceneText:', currentStateForLog.pendingSceneText)
      if (!finalSceneObject) {
      ...
      if (finalSceneObject?.emotionAnalysis) {
        ...
        window.narratorEmotionVector = finalSceneObject.emotionAnalysis?.base || {
          fear: 0,
          sadness: 0,
          anger: 0,
          joy: 0,
...
async function handleExpConfirm(answer) {
  removeExpConfirmButtons()
  const currentScene = appStore.getState().currentScene
  if (answer === 'yes') {
    addExpChatMessage('user', 'Yes')
    if (expFinalObject) {
      addExpChatMessage('ai', 'Converting emotion to wave...')
      await saveExpInterpretation(expFinalObject)
      if (expFinalObject.emotionAnalysis && appStore.getState().currentSessionId) {
        await saveExperiencerChoice(expFinalObject.emotionAnalysis.base)
      }
      if (expFinalObject.emotionAnalysis) {
        window.experiencerEmotionVector = expFinalObject.emotionAnalysis.base
        const experiencerWave = emotionVectorToWaveStyle(expFinalObject.emotionAnalysis.base)
        window.currentExperiencerWave = experiencerWave
        updateAlignmentWave()
        renderExperiencerWave(expFinalObject.emotionAnalysis)
        updateExperiencerAlignment()
      }
      setTimeout(() => checkAlignment(), 1000)
    }
    addExpChatMessage('ai', 'Emotion sent.')
    expCurrentPhase = 'waiting'
    appStore.setState({ expPendingEmotion: '' })
    expGeneratedEmotion = ''
    expFinalObject = null
    ...
    addExpChatMessage('ai', '다음 기억을 기다리고 있어.')
  } else {
    addExpChatMessage('user', 'No')
    addExpChatMessage('ai', '다시 감정을 입력 please.')
...
async function saveExperiencerChoice(emotionVector) {
  console.log('=== saveExperiencerChoice called ===')
  console.log('emotionVector:', JSON.stringify(emotionVector))
  const state = appStore.getState()
  console.log('currentSessionId:', state.currentSessionId)
  console.log('liveSceneNum:', state.liveSceneNum)
  if (!state.currentSessionId) {
    console.error('currentSessionId가 not found!')
    return
  }
  ...
  const insertData = {
    ...
    scene_id: null,
    user_id: userId,
    emotion_vector: emotionVector || { fear: 0, sadness: 0, anger: 0, joy: 0, longing: 0, guilt: 0 },
    created_at: new Date().toISOString(),
  }
  console.log('choices INSERT 데이터:', JSON.stringify(insertData))
  try {
    const result = await networkService.saveChoice(insertData)
    if (!result.ok) {
      console.error('choices INSERT error:', result.error)
      throw result.error
    }
    console.log('체험자 감정 저장 complete:', result.data)
    showNotification('감정이 choices 테이블에 저장 complete')
  } catch (e) {
    console.error('saveExperiencerChoice error:', e)
    showNotification('choices 테이블 Save failed: ' + e.message)
  }
}
  ...
  try {
    ...
    if (sceneText) {
      ...
      if (
        ...
        !currentData.scenes[currentScene]
      ) {
...
function renderLiveEchoLayer(words) {
  const layer = document.getElementById('liveEchoLayer')
  if (!layer) return
  layer.innerHTML = ''
...
function makeLiveChoice(choiceIndex) {
  try {
    const state = appStore.getState()
    ...
    if (choiceIndex === scene.originalChoice) {
      appStore.setState({ liveMatches: updatedState.liveMatches + 1 })
      const matchesEl = document.getElementById('liveMatches')
      if (matchesEl) matchesEl.textContent = updatedState.liveMatches + 1
    }
    appStore.setState({ liveFragments: updatedState.liveFragments + 1 })
    const fragmentsEl = document.getElementById('liveFragments')
    if (fragmentsEl) fragmentsEl.textContent = updatedState.liveFragments + 1
    ...
    if (sceneType === 'branch' || sceneType === 'ending') {
      ...
      window.proceedToNextSceneLive()
    }
  } catch (e) {
    console.error('makeLiveChoice error:', e)
...
function submitExperiencerFeeling() {
  try {
    const feelingInput = document.getElementById('experiencerFeelingInput')
    if (!feelingInput) {
    ...
    if (!feeling) {
      showNotification('Please describe how the narrator might have felt')
      return
    }
    const state = appStore.getState()
    appStore.setState({ userReasons: [...state.userReasons, feeling], liveFragments: state.liveFragments + 1 })
    const updatedState = appStore.getState()
    const fragmentsEl = document.getElementById('liveFragments')
    if (fragmentsEl) fragmentsEl.textContent = updatedState.liveFragments
    updateLiveAlignment(0.1 + Math.random() * 0.15)
    showNotification('Emotion recorded')
    feelingInput.value = ''
    setTimeout(() => {
      window.proceedToNextSceneLive()
    }, 1000)
  } catch (e) {
    console.error('submitExperiencerFeeling error:', e)
    showNotification('An error occurred')
  }
}

function submitScene() {
  console.log('submitScene called')
  const input = document.getElementById('sceneTextInput')
  const submitBtn = document.querySelector('.scene-submit-btn')
  if (!input.value.trim()) {
    showNotification('Please enter a scene')
    return
  }
  currentSceneText = input.value.trim()
  submitBtn.disabled = true
  submitBtn.textContent = 'AI is converting scene...'
  generateSceneAI(currentSceneText)
    .then((aiScene) => {
      const liveSceneContent = document.getElementById('liveSceneContent')
      if (liveSceneContent) {
        liveSceneContent.textContent = aiScene
      }
      const traceContent = document.getElementById('traceContent')
      if (traceContent) {
        traceContent.textContent = 'Scene sent to experiencer'
      }
      showNotification('AI converted and sent the scene')
    })
    .catch((err) => {
      console.error('AI scene generation error:', err)
      showNotification('An error occurred during scene conversion')
      const traceContent = document.getElementById('traceContent')
      if (traceContent) {
        traceContent.textContent = currentSceneText
      }
    })
    .finally(() => {
      input.value = ''
...
function updateLiveAlignment(delta) {
  const state = appStore.getState()
  const newAlignment = Math.min(1, state.currentAlignment + delta)
  appStore.setState({ currentAlignment: newAlignment })
  const updatedState = appStore.getState()
  const liveAlignmentValue = document.getElementById('liveAlignmentValue')
  if (liveAlignmentValue) {
    liveAlignmentValue.textContent = updatedState.currentAlignment.toFixed(2)
    if (updatedState.currentAlignment >= 0.8) liveAlignmentValue.classList.add('high')
  }
  const liveAlignmentFill = document.getElementById('liveAlignmentFill')
  if (liveAlignmentFill) liveAlignmentFill.style.width = updatedState.currentAlignment * 100 + '%'
  const alignmentPercentage = document.getElementById('alignmentPercentage')
  if (alignmentPercentage)
    alignmentPercentage.textContent = String(Math.round(updatedState.currentAlignment * 100)).padStart(2, '0') + '%'
  const expAlignmentPercentage = document.getElementById('expAlignmentPercentage')
  if (expAlignmentPercentage)
    expAlignmentPercentage.textContent = String(Math.round(updatedState.currentAlignment * 100)).padStart(2, '0') + '%'
}
...
function updateNarratorWave(emotionAnalysis) {
  currentNarratorWave = emotionVectorToWaveStyle(emotionAnalysis?.base)
  console.log('화자 파동 update:', currentNarratorWave)
}

function renderExperiencerWave(emotionAnalysis) {
  console.log('체험자 파동 rendering:', emotionAnalysis)
  if (!emotionAnalysis || !emotionAnalysis.base) {
    console.error('emotionAnalysis 또는 base가 not found')
    return
  }
  const emotionVector = emotionAnalysis.base
  window.experiencerEmotionVector = emotionVector
  const experiencerWave = emotionVectorToWaveStyle(emotionAnalysis.base)
  window.currentExperiencerWave = experiencerWave
  updateAlignmentWave()
  console.log('체험자 Wave rendering complete:', experiencerWave)
}
...
function updateExperiencerStatus(status) {
  const floatEl = document.getElementById('experiencerStatusFloat')
  if (!floatEl) return
  floatEl.style.display = 'block'
  floatEl.textContent = status
  experiencerStatusPosition = experiencerStatusPosition === 'left' ? 'right' : 'left'
  floatEl.classList.remove('left', 'right')
  floatEl.classList.add(experiencerStatusPosition)
}
...
function toggleEditMode() {
  const editBtn = document.querySelector('.edit-toggle-btn')
  let textEl, textarea, confirmMsg
  if (currentPhase === 'scene' || currentPhase === 'complete') {
    textEl = document.querySelector('#generatedSceneContent .generated-text')
    textarea = document.getElementById('editSceneTextarea')
    confirmMsg = 'Does this memory feel right?'
  } else if (currentPhase === 'emotion') {
    textEl = document.querySelector('#generatedEmotionContent .generated-text')
    textarea = document.getElementById('editEmotionTextarea')
    confirmMsg = 'Is this what you felt?'
  }
  ...
  if (isEditMode) {
    ...
    editBtn.textContent = 'Edit'
    editBtn.classList.remove('active')
    showNotification('Edit complete')
    if (currentPhase !== 'complete') {
      addChatMessageWithConfirm('ai', confirmMsg)
    }
  }
}

function saveEditedScene() {
  const textarea = document.getElementById('editSceneTextarea')
  if (!textarea || !textarea.value.trim()) {
    showNotification('Please enter your edit')
    return
  }
  if (currentGeneratedSceneObj) {
    currentGeneratedSceneObj.text = textarea.value.trim()
  }
  currentGeneratedScene = textarea.value.trim()
  const sceneContent = document.getElementById('generatedSceneContent').querySelector('.generated-text')
  if (sceneContent) sceneContent.textContent = textarea.value.trim()
  showNotification('Scene edit complete')
}
...
function startVoiceMode() {
  document.getElementById('voiceStartPrompt').style.display = 'none'
...
function switchToTextMode() {
  ...
  isVoiceMode = false
  if (recognizedText) {
    document.getElementById('sceneTextInput').value = recognizedText
  }
}
function startSpeechRecognition() {
  ...
  recognition.start()
  showNotification('음성 인식이 시작 complete')
}
function startVoiceVisualization() {
  navigator.mediaDevices
    ...
    .then(function (stream) {
      ...
      function draw() {
        ...
        for (let i = 0; i < bufferLength; i++) {
          ...
          gradient.addColorStop(0, 'rgba(196,168,130,0.8)')
          gradient.addColorStop(1, 'rgba(196,168,130,0.4)')
    ...
    .catch(function (err) {
      ...
      showNotification('마이크 접근이 거부 complete')
    })
}
...
function switchToTextInput() {
  const waveSection = document.querySelector('.voice-wave-section')
  const switchBtn = document.querySelector('.text-switch-btn')
  if (waveSection && switchBtn) {
    waveSection.style.display = 'none'
    const textInputContainer = document.createElement('div')
    textInputContainer.className = 'text-input-container-live'
    textInputContainer.style.width = '100%'
    textInputContainer.innerHTML = `<div class="chat-input-wrapper"><textarea class="chat-input-textarea" id="liveTextInput" placeholder="Tell your memory..." rows="3"></textarea><button class="chat-send-btn" id="chatSendBtn" onclick="sendChatMessage()">Send</button></div>`
    switchBtn.parentElement.insertBefore(textInputContainer, switchBtn)
...
function switchToVoiceInput() {
  const textContainer = document.querySelector('.text-input-container-live')
  const switchBtn = document.querySelector('.text-switch-btn')
  const waveSection = document.querySelector('.voice-wave-section')
  if (textContainer) {
...
function switchExpToTextInput() {
  if (isExpRecording) {
    if (expMediaRecorder && expMediaRecorder.state !== 'inactive') {
      expMediaRecorder.stop()
      isExpRecording = false
    }
  }
  const waveSection = document.getElementById('expVoiceWaveSection')
  const switchBtn = document.querySelector('.experiencer-panel .text-switch-btn')
  if (waveSection && switchBtn) {
    waveSection.style.display = 'none'
    const textInputContainer = document.createElement('div')
    textInputContainer.className = 'text-input-container-live'
    textInputContainer.style.width = '100%'
    textInputContainer.innerHTML = `<div class="chat-input-wrapper"><textarea class="chat-input-textarea" id="expTextInput" placeholder="Enter your emotion..." rows="3"></textarea><button class="chat-send-btn" id="expChatSendBtn" onclick="sendExpChatMessage()">Send</button></div>`
    switchBtn.parentElement.insertBefore(textInputContainer, switchBtn)
    switchBtn.textContent = 'Switch to Voice'
    switchBtn.onclick = function () {
      switchExpToVoiceInput()
    }
    const input = document.getElementById('expTextInput')
    if (input) {
      input.addEventListener('keydown', function (e) {
        if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
          e.preventDefault()
...
function switchExpToVoiceInput() {
  const textContainer = document.querySelector('.experiencer-panel .text-input-container-live')
  const switchBtn = document.querySelector('.experiencer-panel .text-switch-btn')
  const waveSection = document.getElementById('expVoiceWaveSection')
  if (textContainer) {
    textContainer.remove()
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        if (audioChunks.length === 0) {
          ...
          showNotification('No recording found')
        ...
        if (text) {
          const sceneInput = document.getElementById('liveTextInput')
          if (sceneInput) {
          ...
          showNotification('Voice conversion failed')
        }
        btn.textContent = '🎤 Voice Input'
        mediaRecorder.stream.getTracks().forEach((track) => track.stop())
      }
      ...
      showNotification('녹음이 시작 complete')
    } catch (err) {
      console.error('녹음 시작 error:', err)
      alert('마이크 권한이 필요합니다')
      btn.textContent = '🎤 Voice Input'
    }
  } else {
...
async function transcribeAudio(audioBlob) {
  try {
    ...
    const { data, error } = await supabaseClient.functions.invoke('transcribe-audio', { body: formData })
    if (error) {
      console.error('Supabase Edge Function error:', error)
      throw error
    }
    if (!data || !data.text) {
      console.error('응답 데이터 형식 error:', data)
      throw new Error('Invalid response data format')
    }
    const text = data.text
    let sceneInput = document.getElementById('liveTextInput')
    if (!sceneInput) {
      const switchBtn = document.querySelector('.text-switch-btn')
      if (switchBtn && typeof switchToTextInput === 'function') {
        switchToTextInput()
        sceneInput = document.getElementById('liveTextInput')
      }
    }
    if (sceneInput) {
      sceneInput.value = text
    ...
    console.error('음성 변환 에러:', err)
    const errorMsg = err.message || 'Voice conversion failed'
    showNotification(errorMsg)
    return null
  }
}

async function transcribeExpAudio(audioBlob) {
  try {
    const supabaseClient = getSupabaseClient()
    if (!supabaseClient) {
      throw new Error('Supabase client not initialized')
    ...
    const text = data.text
    let expInput = document.getElementById('expTextInput')
    if (!expInput) {
      showNotification('Input field not found')
      return null
    }
...
async function toggleExpRecording(e) {
  if (e) e.stopPropagation()
  const btn = document.getElementById('expVoiceBtn')
  if (!btn) return
  if (!isExpRecording) {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true })
      expMediaRecorder = new MediaRecorder(stream)
      expAudioChunks = []
      expMediaRecorder.ondataavailable = (e) => {
        expAudioChunks.push(e.data)
      }
      expMediaRecorder.onstop = async () => {
        if (expAudioChunks.length === 0) {
          btn.textContent = '🎤 Voice Input'
          showNotification('No recording found')
          expMediaRecorder.stream.getTracks().forEach((track) => track.stop())
          return
        }
        const audioBlob = new Blob(expAudioChunks, { type: 'audio/webm' })
        btn.textContent = '⏳ Converting...'
        const text = await transcribeExpAudio(audioBlob)
        if (text) {
          showNotification('Voice input complete')
        } else {
          showNotification('Voice conversion failed')
        }
        btn.textContent = '🎤 Voice Input'
...
function startLiveVoiceInput() {
  ...
  liveVoiceRecognition.onend = function () {
    if (isLiveVoiceRecording && liveVoiceRecognition) {
  ...
  navigator.mediaDevices
    .getUserMedia({ audio: true })
    .then(function (stream) {
      liveVoiceContext = new (window.AudioContext || window.webkitAudioContext)()
      liveVoiceAnalyser = liveVoiceContext.createAnalyser()
      liveVoiceMicrophone = liveVoiceContext.createMediaStreamSource(stream)
      liveVoiceMicrophone.connect(liveVoiceAnalyser)
      liveVoiceAnalyser.fftSize = 256
      const bufferLength = liveVoiceAnalyser.frequencyBinCount
      const dataArray = new Uint8Array(bufferLength)
      const canvas = document.getElementById('voiceWaveCanvasLive')
      const ctx = canvas.getContext('2d')
      canvas.width = canvas.offsetWidth * 2
      canvas.height = canvas.offsetHeight * 2
      ctx.scale(2, 2)
      function draw() {
        if (!isLiveVoiceRecording) return
        liveVoiceAnimationId = requestAnimationFrame(draw)
        liveVoiceAnalyser.getByteFrequencyData(dataArray)
        ...
        for (let i = 0; i < bufferLength; i++) {
          ...
          gradient.addColorStop(0, 'rgba(122,154,122,0.9)')
          gradient.addColorStop(1, 'rgba(74,144,217,0.5)')
          ctx.fillStyle = gradient
  ...
  const waveSection = document.querySelector('.voice-wave-section')
  if (waveSection) waveSection.style.border = '2px solid rgba(122,154,122,0.5)'
  showNotification('음성 입력이 시작 complete. 말한 내용은 자동으로 전송됩니다.')
}

function stopLiveVoiceInput() {
  if (!isLiveVoiceRecording && !liveVoiceRecognition) return
  isLiveVoiceRecording = false
  if (liveVoiceRecognition) {
    liveVoiceRecognition.stop()
    liveVoiceRecognition = null
  }
  if (liveVoiceAnimationId) {
    cancelAnimationFrame(liveVoiceAnimationId)
    liveVoiceAnimationId = null
  }
  if (liveVoiceContext) {
    liveVoiceContext.close()
    liveVoiceContext = null
  }
  liveVoiceAnalyser = null
  liveVoiceMicrophone = null
  if (liveRecognizedText.trim()) {
    const input = document.getElementById('liveTextInput')
    if (input) {
      input.value = liveRecognizedText.trim()
      sendChatMessage()
      liveRecognizedText = ''
    } else {
      addChatMessage('user', liveRecognizedText.trim())
      conversationHistory.push({ role: 'user', content: liveRecognizedText.trim() })
      callClaudeAPI(liveRecognizedText.trim())
        .then((aiResponse) => {
          addChatMessage('ai', aiResponse)
          conversationHistory.push({ role: 'assistant', content: aiResponse })
          if (aiResponse.includes('[SCENE_READY]')) {
            parseAndGenerateScene(aiResponse)
          }
        })
        .catch((error) => {
          console.error('AI API error:', error)
          addChatMessage('ai', 'Sorry, something went wrong. Could you say that again?')
        })
      liveRecognizedText = ''
    }
  }
  const waveSection = document.querySelector('.voice-wave-section')
  if (waveSection) waveSection.style.border = 'none'
  startVoiceWaveLiveAnimation()
  showNotification('음성 입력이 중지 complete')
}

async function callClaudeAPI(userMessage) {
  try {
    const messages = conversationHistory.length > 0 ? conversationHistory : [{ role: 'user', content: userMessage }]
    ...
    const response = await fetch(SUPABASE_FUNCTION_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${SUPABASE_ANON_KEY}` },
      body: JSON.stringify({ text: userMessage, conversationHistory: messages, systemPrompt: AI_SYSTEM_PROMPT }),
    })
    if (!response.ok) {
      const error = await response.json()
      throw new Error(error.error || error.details || 'API call failed')
...
function parseAndGenerateScene(aiResponse) {
  try {
    const sceneReadyIndex = aiResponse.indexOf('[SCENE_READY]')
    if (sceneReadyIndex === -1) return
    const jsonStr = aiResponse.substring(sceneReadyIndex + '[SCENE_READY]'.length).trim()
    const sceneData = JSON.parse(jsonStr)
    currentGeneratedSceneObj = sceneData.scene
    currentGeneratedEmotion = sceneData.emotion
    if (sceneData.scene && sceneData.scene.text) {
      currentGeneratedScene = sceneData.scene.text
    }
    updateGeneratedTabs(sceneData)
    updateAlignmentFromScene(sceneData)
    showNotification('Scene generated')
  } catch (error) {
...
function updateGeneratedTabs(sceneData) {
  if (sceneData.scene && sceneData.scene.text) {
    const sceneContent = document.getElementById('generatedSceneContent').querySelector('.generated-text')
    if (sceneContent) {
      sceneContent.textContent = sceneData.scene.text
      sceneContent.classList.remove('void-scene')
    }
    const editTextarea = document.getElementById('editSceneTextarea')
    if (editTextarea) editTextarea.value = sceneData.scene.text
  }
  if (sceneData.emotion && sceneData.emotion.text) {
    const emotionContent = document.getElementById('generatedEmotionContent').querySelector('.generated-text')
    if (emotionContent) {
      emotionContent.textContent = sceneData.emotion.text
      emotionContent.classList.remove('void-reason')
    }
  }
  if (sceneData.voidInfo) {
    const sceneContent = document.getElementById('generatedSceneContent').querySelector('.generated-text')
    const emotionContent = document.getElementById('generatedEmotionContent').querySelector('.generated-text')
    if (sceneData.voidInfo.sceneVoid && sceneContent) {
      sceneContent.classList.add('void-scene')
    }
    if (sceneData.voidInfo.reasonVoid && emotionContent) {
      emotionContent.classList.add('void-reason')
    }
  }
}
  ...
  function animateWave(canvas, isExperiencer) {
    ...
    try {
      ...
      for (let x = 0; x < width; x++) {
        const y =
          centerY + Math.sin(x * 0.03 + time * 0.08) * amplitude + Math.sin(x * 0.015 + time * 0.05) * (amplitude * 0.6)
...
function stopVoiceWaveLiveAnimation() {
  if (voiceWaveLiveAnimationId) {
    cancelAnimationFrame(voiceWaveLiveAnimationId)
    voiceWaveLiveAnimationId = null
  }
}
