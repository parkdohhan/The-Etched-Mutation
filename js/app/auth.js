 ...
 *   appStore, window.showNotification, window.saveMemory,
 *   window.handleCrisis
 */
...
async function handleLogout() { if (confirm('Are you sure you want to sign out?')) { supabaseClient = getSupabaseClient(); if (!supabaseClient) { window.showNotification('Supabase client not initialized'); return } await supabaseClient.auth.signOut(); appStore.setState({ isLoggedIn: false, currentUser: null }); closeMypage(); window.showNotification('Signed out successfully') } }
