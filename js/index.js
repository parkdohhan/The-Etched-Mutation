...
async function saveLiveScene(sceneData){console.log('=== saveLiveScene 호출 ===');console.log('sceneData:',JSON.stringify(sceneData));console.log('sceneData.emotionAnalysis:',JSON.stringify(sceneData.emotionAnalysis));console.log('sceneData.emotionAnalysis?.base:',JSON.stringify(sceneData.emotionAnalysis?.base));console.log('currentSessionId:',currentSessionId);console.log('liveSceneNum:',liveSceneNum);console.log('currentGeneratedScene:',currentGeneratedScene);supabaseClient = getSupabaseClient(); if(!supabaseClient){console.error('supabaseClient가 없습니다!');showNotification('Supabase 클라이언트가 초기화되지 않았습니다');return}if(!currentSessionId){console.error('currentSessionId가 없습니다!');showNotification('세션이 없습니다');return}const sceneText=sceneData.text||currentGeneratedScene||pendingSceneText||'';if(!sceneText||sceneText==='(장면 없음)'){console.error('장면 텍스트가 없습니다!');showNotification('저장할 장면이 없습니다');return}const emotionVectorToSave=sceneData.emotionAnalysis?.base||{fear:0,sadness:0,anger:0,joy:0,longing:0,guilt:0};console.log('=== 저장할 감정 벡터 ===');console.log('emotionVectorToSave:',JSON.stringify(emotionVectorToSave));const insertData={session_id:currentSessionId,scene_index:liveSceneNum,scene_text:sceneText,emotion_raw:sceneData.emotionRaw||'',reason_raw:sceneData.reasonRaw||'',generated_emotion:sceneData.generatedEmotion||'',emotion_vector:emotionVectorToSave,intensity:sceneData.emotionAnalysis?.intensity||0.5,confidence:sceneData.emotionAnalysis?.confidence||0.5,void_scene:sceneData.voidInfo?.sceneVoid||false,void_emotion:sceneData.voidInfo?.emotionVoid||false,void_reason:sceneData.voidInfo?.reasonVoid||false};console.log('insertData:',JSON.stringify(insertData));console.log('insertData.emotion_vector:',JSON.stringify(insertData.emotion_vector));try{const{data,error}=await supabaseClient.from('live_scenes').insert(insertData).select();if(error){console.error('live_scenes INSERT error:',error);throw error}console.log('live_scenes 저장 성공:',data);await saveSceneToLiveSession({text:sceneText,emotion_vector:sceneData.emotionAnalysis?.base||{fear:0,sadness:0,anger:0,joy:0,longing:0,guilt:0}});showNotification('장면이 체험자에게 전송되었습니다')}catch(e){console.error('saveLiveScene error:',e);showNotification('장면 저장 실패: '+e.message)}}
...
async function handleConfirm(answer){
    ...
    if(currentPhase==='emotion'){
        if(answer==='yes'){
            ...
            console.log('finalSceneObject.emotionAnalysis:',JSON.stringify(finalSceneObject.emotionAnalysis));
            console.log('finalSceneObject.emotionAnalysis?.base:',JSON.stringify(finalSceneObject.emotionAnalysis?.base));
...
function fadeInSound(audio,targetVolume=0.6,duration=4000){if(!audio){console.warn('fadeInSound: audio is null');return}if(audio.readyState===0){console.warn('fadeInSound: audio not loaded, readyState:',audio.readyState);audio.load()}audio.volume=0;const playPromise=audio.play();if(playPromise!==undefined){playPromise.then(()=>{console.log('Audio started playing successfully')}).catch(e=>{if(e.name==='NotAllowedError'){console.warn('Audio play blocked by browser policy - user interaction required')}else if(e.name!=='AbortError'){console.error('Audio play failed:',e)}})}const steps=60;const step=targetVolume/steps;const interval=duration/steps;let currentStep=0;const fade=setInterval(()=>{if(!audio){clearInterval(fade);return}currentStep++;if(currentStep<steps){audio.volume=Math.min(1,Math.max(0,Math.min(step*currentStep,targetVolume)))}else{audio.volume=Math.min(1,Math.max(0,targetVolume));clearInterval(fade)}},interval)}
function fadeOutSound(audio,duration=3000){if(!audio)return;if(fadeOutAnimationId){cancelAnimationFrame(fadeOutAnimationId);fadeOutAnimationId=null}if(crossfadeTimeUpdateHandler&&audio){audio.removeEventListener('timeupdate',crossfadeTimeUpdateHandler);crossfadeTimeUpdateHandler=null}if(crossfadeEndedHandler&&audio){audio.removeEventListener('ended',crossfadeEndedHandler);crossfadeEndedHandler=null}const startVolume=Math.max(audio.volume||0,0.01);if(startVolume<=0){try{audio.pause();audio.currentTime=0}catch(e){}return}if(audio.paused){const playPromise=audio.play();if(playPromise!==undefined){playPromise.catch(e=>{if(e.name!=='AbortError'&&e.name!=='NotAllowedError'){}})}}const startTime=performance.now();let lastVolume=startVolume;let pauseCheckInterval=setInterval(()=>{if(audio&&audio.paused&&lastVolume>0.01){const playPromise=audio.play();if(playPromise!==undefined){playPromise.catch(e=>{if(e.name!=='AbortError'&&e.name!=='NotAllowedError'){}})}}},50);function animateFadeOut(currentTime){if(!audio){if(fadeOutAnimationId){cancelAnimationFrame(fadeOutAnimationId);fadeOutAnimationId=null}if(pauseCheckInterval){clearInterval(pauseCheckInterval);pauseCheckInterval=null}return}if(audio.paused&&lastVolume>0.01){const playPromise=audio.play();if(playPromise!==undefined){playPromise.catch(e=>{if(e.name!=='AbortError'&&e.name!=='NotAllowedError'){}})}}const elapsed=currentTime-startTime;const progress=Math.min(elapsed/duration,1);const newVolume=Math.max(startVolume*(1-progress),0);lastVolume=newVolume;try{if(audio){audio.volume=Math.min(1,Math.max(0.001,Math.max(newVolume,0.001)))}}catch(e){}if(progress>=1||newVolume<=0.01){if(pauseCheckInterval){clearInterval(pauseCheckInterval);pauseCheckInterval=null}setTimeout(()=>{try{if(audio){audio.volume=0;audio.pause();audio.currentTime=0}}catch(e){}if(fadeOutAnimationId){cancelAnimationFrame(fadeOutAnimationId);fadeOutAnimationId=null}},200)}else{fadeOutAnimationId=requestAnimationFrame(animateFadeOut)}}fadeOutAnimationId=requestAnimationFrame(animateFadeOut)}
...
document.addEventListener('DOMContentLoaded',function(){openingSound=document.getElementById('openingSound');if(openingSound){console.log('Opening sound element found, src:',openingSound.src,'readyState:',openingSound.readyState);openingSound.addEventListener('error',function(e){console.warn('Opening sound file not found or failed to load:',openingSound.src,'error:',e);console.warn('Make sure the file exists at:',openingSound.src)});openingSound.addEventListener('loadeddata',function(){console.log('Opening sound loadeddata event, readyState:',openingSound.readyState)});openingSound.addEventListener('canplay',function(){console.log('Opening sound canplay event, readyState:',openingSound.readyState)})}else{console.error('Opening sound element not found!')}const openingScreen=document.getElementById('openingScreen');if(openingScreen){openingScreen.style.cssText='display:flex !important;visibility:visible !important;opacity:1 !important;pointer-events:auto !important;z-index:3000 !important'}const waveContainer=document.getElementById('openingWaveContainer');if(waveContainer){waveContainer.classList.add('visible');const canvas=document.getElementById('openingWaveCanvas');if(canvas)startOpeningWaveAnimation(canvas)}const hint=document.getElementById('openingStartHint');if(hint){hint.style.opacity='1';hint.classList.add('visible')}});
function startOpeningSound(){if(!openingSound){console.warn('startOpeningSound: openingSound is null');return}const audioSrc=openingSound.querySelector('source')?.src||openingSound.src;if(!audioSrc||audioSrc.trim()===''){console.warn('startOpeningSound: No audio source specified. Audio file is missing.');return}console.log('startOpeningSound: readyState=',openingSound.readyState,'src=',audioSrc);let hasError=false;const errorHandler=function(e){if(!hasError){hasError=true;console.warn('startOpeningSound: Audio file not found or failed to load:',audioSrc);console.warn('Please add the audio file at: sounds/ambient-opening.mp3 or remove the audio element from HTML')}openingSound.removeEventListener('error',errorHandler)};openingSound.addEventListener('error',errorHandler,{once:true});if(openingSound.readyState>=2){console.log('startOpeningSound: ready, starting immediately');setupLoopWithCrossfade(openingSound,0.6,2);fadeInSound(openingSound,0.6,4000)}else if(openingSound.readyState===0){console.log('startOpeningSound: not loaded, loading...');openingSound.load();openingSound.addEventListener('canplay',function startSound(){if(!hasError){console.log('startOpeningSound: canplay event fired');openingSound.removeEventListener('canplay',startSound);setupLoopWithCrossfade(openingSound,0.6,2);fadeInSound(openingSound,0.6,4000)}},{once:true})}else{console.log('startOpeningSound: waiting for canplay, readyState=',openingSound.readyState);openingSound.addEventListener('canplay',function startSound(){if(!hasError){console.log('startOpeningSound: canplay event fired');openingSound.removeEventListener('canplay',startSound);setupLoopWithCrossfade(openingSound,0.6,2);fadeInSound(openingSound,0.6,4000)}},{once:true})}}const openingScreenEl=document.getElementById('openingScreen');if(openingScreenEl){openingScreenEl.addEventListener('click',function(e){if(hasZoomedIn){skipToIntro();return}hasZoomedIn=true;const waveContainer=document.getElementById('openingWaveContainer');if(waveContainer){waveContainer.style.transform='scale(1)';waveContainer.style.opacity='1'}startOpeningSound();const hint=document.getElementById('openingStartHint');if(hint)hint.style.opacity='0';setTimeout(()=>{if(!openingSequenceStarted){openingSequenceStarted=true;startOpeningSequence()}},800)})}document.addEventListener('keydown',function(e){if(!openingSkipped&&!hasZoomedIn){e.preventDefault();hasZoomedIn=true;const waveContainer=document.getElementById('openingWaveContainer');if(waveContainer){waveContainer.style.transform='scale(1)';waveContainer.style.opacity='1'}startOpeningSound();const hint=document.getElementById('openingStartHint');if(hint)hint.style.opacity='0';setTimeout(()=>{if(!openingSequenceStarted){openingSequenceStarted=true;startOpeningSequence()}},800)}else if(!openingSkipped&&hasZoomedIn){handleOpeningKeydown(e)}});
