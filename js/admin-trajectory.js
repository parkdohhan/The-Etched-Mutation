...
function bindToggles() {
  ...
  // 260730 — "사물 3D 모델 뽑기": 서버에 주문+수확을 시킨다 (generate-object-model).
  //   한 번 누르면 완성된 것을 거둬들고, 아직 모델 없는 단어는 새로 주문한다.
  //   모델 하나당 60~120초라 즉시 다 나오지 않는다 — 잠시 뒤 다시 누르면 나머지가 붙는다.
  ...
  if (objModelsBtn) objModelsBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const parts = [];
      if (data.collected?.length) parts.push(`거둠 ${data.collected.length}: ${data.collected.join(', ')}`);
      if (data.queued?.length) parts.push(`주문 ${data.queued.length}: ${data.queued.join(', ')} (1~2분 뒤 다시 누르면 붙음)`);
      if (data.running?.length) parts.push(`굽는 중 ${data.running.length}: ${data.running.join(', ')}`);
      if (data.deferred?.length) parts.push(`대기 ${data.deferred.length}: ${data.deferred.join(', ')}`);
      if (data.failed?.length) parts.push(`실패 ${data.failed.length}: ${data.failed.join(', ')}`);
      if (data.rejected?.length) parts.push(`거절 ${data.rejected.length}: ${data.rejected.join(', ')}`);
      parts.push(`합 완성 ${data.models_total ?? 0}개 / 진행 ${data.jobs_pending ?? 0}개`);
      say(parts.join(' · '), (data.failed?.length || data.rejected?.length) ? '#b8905a' : '#6aa383');
      syncStageView();   // 새로 붙은 모델을 무대 마커에도 반영하게
    } catch (e) {
...
function renderDetailTab(s) {
  ...
  return `
    ...
    <div style="display:flex;gap:6px;margin-top:4px;align-items:center;">
      ...
      <input type="number" id="sceneSoundRadius" min="1" max="100" step="1" value="${s.meta?.sound_radius ?? 15}" style="width:60px;padding:3px 6px;background:rgba(20,20,28,0.8);border:1px solid rgba(196,168,130,0.12);color:#e0d8c4;font-size:0.75rem;border-radius:2px;" title="들리는 한계 거리 — 반경 안쪽 1/4까지 최대 볼륨, 반경에서 0 (밖 = 무음)" />
