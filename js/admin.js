...
function editMemory(index) {
 ...
 // R1-7: sound_map 5칸 UI 제거됨 — 로드 코드도 함께 정리 (DB 값은 보존, UI만 없음)
    ...
    // 260730 검수: SceneNavigator 진단기 삭제 — 부르는 버튼도 그릴 DOM도 0 (도달 불가 코드였음)
    ...
    currentScenes.forEach((scene, sceneIndex) => {
        ...
        sceneBlock.innerHTML = `
            ...
            <div class="editor-section scene-original-fields adv-only" data-scene-index="${sceneIndex}" style="display: ${(scene.sceneType === 'branch' || scene.sceneType === 'ending') ? 'block' : 'none'}; margin-top: 1.5rem; padding: 1.5rem; background: var(--bg-surface); border: 1px solid rgba(196, 168, 130, .2); border-radius: 4px;">
                ...
                <div class="editor-input-group adv-only" style="margin-bottom: 1.5rem;">
                    <label class="editor-label">원본 이유 벡터 (Reason Vector) — 상영 미사용 ⚠️ 유일 리더가 고전 씬 UI 안(도달 불가), 260730 검수</label>
...
function switchTab(tab) {
    ...
    // 260730 검수: 'utterances' 분기 제거 — 호출처 0 + 탭 인덱스 어긋남(W2-G에서 운영 탭으로 이관).
}
...
async function saveMemory() {
 ...
 // R1-7 (2026-07-14): 저장 시 JSON 자동 다운로드 제거 — (260730: 수동 내보내기도 삭제됨)
...
// 260730 검수: convertToMemoriesDataFormat/exportMemoriesJSON/importMemoriesJSON 삭제 — 6축 옛 형식(안내문 허위·DB 미반영)
// local 스토리지 save
...
// 260730 검수: SceneNavigator 진단기(runSceneNavDiagnostics + _renderSceneGraph ~515줄) 삭제 — 도달 불가 코드
...
// 260730 검수: exportMemoriesJSON/importMemoriesJSON 제거 (6축 옛 형식·안내문 허위·DB 미반영)
