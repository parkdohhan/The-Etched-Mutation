...
function draw2DTerrain(canvas, contaminations) {
  ...
  for (let i = 0; i <= n; i++) {
    const x = (i / n) * w;
    const em = scenes[i]?.original_emotion || {};
    const v = (em.anger || 0) - (em.sadness || 0) * 0.5;
    const y = baseY - v * 15;
  ...
  contaminations.forEach((c) => {
    ...
    if (effect === 'fade') {
      ...
      ctx.beginPath();
      ctx.arc(midX, baseY - 5, 4, 0, Math.PI * 2);
      ctx.fill();
    }
  });

  const playerX = (demoState.sceneIndex / n) * w + segmentW / 2;
  ctx.shadowColor = 'rgba(196,168,130,0.8)';
  ctx.shadowBlur = 10;
  ctx.fillStyle = 'rgba(196,168,130,0.9)';
  ctx.beginPath();
  ctx.arc(playerX, baseY - 5, 6, 0, Math.PI * 2);
  ctx.fill();
  ctx.shadowBlur = 0;
}
