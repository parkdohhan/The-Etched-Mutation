...
- 영어 입력도 같은 규칙이다: "you must have been…", "that must have hurt", "did you ever…?" 같은 헤아림·확인 문형을 관객 자신의 냉소나 거리감으로 뒤집어 읽지 마라. 명백한 sarcasm 표지가 없는 한 공감·애틋함 쪽으로 읽는다.
...
` : '';
      ...
      // 260805 EN 데모 대비 — 영어 매핑 예시·영어 헤아림 문형 규칙·is_void 영어 예시 추가.
      //   한국어 규칙·예시는 문자 그대로 보존 (리트머스 260728 검증 결 유지). 영어판 리트머스 재검증 전까지 EN 판정은 잠정.
...
- 명시적 감정 단어가 있으면 해당 감정은 절대 0이 아님!
- 입력은 한국어·영어·혼용 어떤 언어여도 된다. 번역을 거치지 말고 의미를 직접 읽어라.
- "scared", "terrified", "afraid" → fear를 0.7 이상으로
- "sad", "cried", "grief", "sorrow" → sadness를 0.7 이상으로
- "missed her", "miss you", "wish I could see" → longing을 0.7 이상으로
- "angry", "furious", "pissed" → anger를 0.7 이상으로
- "guilty", "I'm sorry", "my fault" → guilt를 0.7 이상으로
...
3. is_void (공백 여부)
   - true: "모르겠어", "말하고 싶지 않아", "기억 안 나", "I don't know", "can't remember", "don't want to talk about it", 또는 빈 입력("")
...
**중요**: base 객체에는 요청된 앵커 목록(${emotionList})의 모든 감정에 대해 수치를 포함해야 합니다.`;
    ...
    if (body.type === "typo_correct") {
      ...
      // 260805 EN 데모 대비 — 한국어 전용 → 한/영 겸용 (번역 금지 규칙 포함)
      const prompt = `다음 문장의 **명백한 오타와 띄어쓰기**만 교정해라. 입력은 한국어 또는 영어이며, 입력 언어를 그대로 유지한다.

원문: """${text}"""

규칙:
- 의미를 바꾸거나 단어를 추가/삭제/풀어쓰기 하지 말 것. **절대 번역하지 말 것**
- 인물 이름, 고유명사, 의도된 비표준어("머가", "넘 좋아" 같은 구어체 표현, "gonna", "kinda" 같은 영어 구어체)는 그대로 둘 것
- 자모 인접키 오타(예: "안ㄴ녕" → "안녕"), 받침 누락(예: "햇어" → "했어"), 영어 철자 오타(예: "teh" → "the", "remmeber" → "remember"), 어색한 띄어쓰기만 수정
- 교정할 게 없으면 원문 그대로 출력
...
- 설명·따옴표·접두사 없이 **교정된 문장 한 줄만** 출력`;
      ...
      const systemPrompt = `오타 교정기 (한국어·영어). 원문의 언어·톤·어휘·뉘앙스 유지. 번역 금지. 메타 응답 금지. 교정문 한 줄만 출력 (교정 불가 시 원문 그대로).`;
