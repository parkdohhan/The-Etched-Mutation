...
//
// 2026-07-14 R3-4: 배포본(v3)만 갖고 있던 v2 로직을 저장소에 역반영 = 저장소가 정본.
//   역반영: 규칙 7("..." 남용 자제) / TONE 문자열 선두 "..." 제거 / 예시 1개 추가 /
//           응답 후처리 reply.replace(/^\.{2,}\s*/, '').
//   동시 수정: 배포본에만 있던 **깨진 한글 3곳**은 정상 표기로 되돌림 (L3-04).
//     "그러셍을 지도"        → "그랬을지도"
//     "씀 컨텍스트"          → "씬 컨텍스트"
//     "엄마 손이 그러죠 것 같아" → "엄마 손이 그랬던 것 같아"
//   (LLM 이 깨진 지시문을 그대로 읽고 있었다. 깨진 채로 옮기면 안 된다.)
//
// 배경:
...
7. 말줄임표 (...) 남용 자제. 응답 시작에 ... 붙이지 말 것. 귀한 자리에만 가끔 (5번 중 1번 정도).
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 응답 시작 ... 강제 제거 (LLM 이 규칙 7 을 어기고 붙일 때)
    reply = reply.replace(/^\.{2,}\s*/, '').trim();
