...
답변에 백틱으로 감싼 식별자가 등장하면 **그 자리에서 즉시** 괄호 안 짧은 설명을 붙인다. 다음 단락으로 미루지 않는다.

**자주 빼먹는 자리 (모두 위반):**

1. **표 셀** — 셀 폭 좁다고 빼지 말 것. 좁아도 괄호 설명 무조건.
2. **함수 시그니처 단독** — `함수명(파라미터)` 자체는 코드 시그니처지 한국어 설명이 아님. 시그니처 다음에 dash 로 풀어쓰는 패턴 = 룰 우회.
3. **dash 로 설명 분리** — `식별자` — 설명 패턴 금지. 반드시 `식별자`(괄호 안 짧은 설명) 다음에 자유 문장.

**나쁨 (실제 위반 사례, 2026-05-03 발생):**
- `scoreCard(seeker, target, weights)` — 5항 가중 합산 점수 + breakdown
- 표 안 `pickGhostVariant` — V2.1 명제 시연 자리
- `runnerUpDelta` — 1등/2등 점수 차

**좋음 (룰 준수):**
- `scoreCard`(카드 두 장 비교 점수 함수)는 5항 가중 합산으로 0~한자릿수 점수를 뽑는다.
- 표 안 `pickGhostVariant`(유령 변주 1등 선택) | V2.1 명제 시연 자리
- `runnerUpDelta`(1등과 2등 점수 차)는 V2-4 임계 판정 입력.

**셀프 체크 (답변 보내기 전 무조건):** 백틱 안의 모든 식별자 직후에 괄호 설명이 붙어 있는가? 하나라도 빠지면 **답변 다시 작성**.

**효과 중심 풀어쓰기 동반:** 식별자(설명) 만 나열하고 끝내지 말 것. 그것만으로는 *그래서 게임에서/시스템에서 무슨 일이 일어나는지* 가 안 보임. 사용자가 *알아들을 수 있는* 한국어 문장을 식별자(설명) 다음에 붙인다.
